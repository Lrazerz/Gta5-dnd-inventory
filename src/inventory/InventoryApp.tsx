...
const InventoryApp = React.memo(function InventoryApp() {
  ...
  const spaceDownHandler = (e) => {
    ...
    if (e.code.toLowerCase() === 'space') {
      ...
      if (draggedItem && draggedItem.width > 1 && draggedItem.height > 1) {
        dispatch(rotateItem());
      } else if (hoveredItem && hoveredItemArea !== 3 && hoveredItem.width > 1 && hoveredItem.height > 1) {
  ...
  return (
    <InventoryAppStateless
      contextMenu={contextMenu}
      tooltipStyles={tooltipStyles}
      onTooltipMouseOver={tooltipMouseOverHandler}
      onContainersMouseOver={removeCurrentHoveredItem}
    />
  );
});
