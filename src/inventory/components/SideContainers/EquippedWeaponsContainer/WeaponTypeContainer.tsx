...
const WeaponTypeContainer: React.FC<Props> = React.memo(function WeaponTypeContainer({ typeTitle, cells }) {
  ...
  const weaponSquareContent = weaponCell.cell.item ? (
    <SquareEquippedItem key={weaponCell.id} item={weaponCell.cell.item} />
  ) : null;

  const ammoSquareContent = ammoCell.cell.item ? (
    <SquareEquippedItem key={ammoCell.id} item={ammoCell.cell.item} />
  ) : null;
  ...
  const toolsSquares = toolsCells.map((toolsCell) => {
    ...
    if (toolsCell.cell.item) {
      toolsSquareContent = <SquareEquippedItem key={toolsCell.cell.item.id} item={toolsCell.cell.item} />;
    }

    return (
      <div key={toolsCell.id} style={{ width: '19.85%', position: 'relative' }}>
        <Octagon coords={toolsCell.id}>
          <ClosingWeaponSquare coords={toolsCell.id}>{toolsSquareContent}</ClosingWeaponSquare>
        </Octagon>
      </div>
    );
  });
  ...
  return (
    <div className={classes.WeaponTypeContainer}>
      ...
      <div className={classes.WeaponSquareContainer} onMouseOver={(e) => e.stopPropagation()}></div>
      <div className={classes.TransparentHoveredEl} onMouseOver={(e) => e.stopPropagation()}>
        <ClosingWeaponSquare coords={weaponCell.id}>{weaponSquareContent}</ClosingWeaponSquare>
      </div>
      <div className={classes.AttachmentsSquaresContainer}>
        <div className={classes.LeftArrow} />
        <div style={{ width: '19.85%', position: 'relative' }}>
          <Octagon coords={ammoCell.id}>
            <ClosingWeaponSquare coords={ammoCell.id}>{ammoSquareContent}</ClosingWeaponSquare>
          </Octagon>
        </div>
        {toolsSquares}
      </div>
