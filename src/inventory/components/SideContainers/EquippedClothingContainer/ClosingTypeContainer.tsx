...
const ClosingTypeContainer: React.FC<Props> = React.memo(function ClosingTypeContainer({
  typeTitle,
  typeImage,
  cells,
  typeImageWidth,
  ...
  const squaresContent = cells.map(({ id, cell }) => {
    ...
    return (
      <div key={id} style={{ width: '26%', position: 'relative' }}>
        <Octagon coords={id}>
          <ClosingWeaponSquare coords={id}>{squareContent}</ClosingWeaponSquare>
        </Octagon>
      </div>
    );
  });
  ...
  return (
    <div className={classes.ClosingTypeContainer}>
      <div className={classes.TitleAndImageContainer}>
        ...
        <LeadText styles={leadTextStyles}>&nbsp;{typeTitle.toUpperCase()}</LeadText>
      </div>
