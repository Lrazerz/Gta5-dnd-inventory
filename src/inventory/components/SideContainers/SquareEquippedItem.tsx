...
const SquareEquippedItem: React.FC<Props> = React.memo(function SquareEquippedItem({ item }) {
  ...
  const additionalImageContainerStyles: CSSProperties = {
    ...
    >
      ...
      {item.currentCount > 1 && (
        <div className={classes.CurrentCount}>
          <SecondaryText>{item.currentCount}</SecondaryText>
        </div>
      )}
    </div>
  );
  ...
  return <EquippedItem>{item.imageUrl && imageElement}</EquippedItem>;
});
