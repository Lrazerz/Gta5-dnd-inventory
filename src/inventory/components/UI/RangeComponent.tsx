...
import classes from '../../../styles/inventory/UI/RangeComponent.module.scss';
...
} from '../../../redux/actions/inventory/externalBoard';
...
const RangeComponent: React.FC<Props> = React.memo(
  ({ leftOffset, topOffset, contextItem, hoveredArea, splitMenuOpen }) => {
    ...
    const successButtonMouseDownHandler = (event) => {
      ...
      const contextItemWithChangedCount = {
        ...contextItem,
        currentCount: Number(splittedCount),
      };
      ...
      if (hoveredArea === 1) {
        requiredItem = document.getElementById(
          `square-common-item-${contextItem.mainCell[0]}-${contextItem.mainCell[1]}`,
        );
      } else if (hoveredArea === 2) {
        requiredItem = document.getElementById(
          `external-square-common-item-${contextItem.mainCell[0]}-${contextItem.mainCell[1]}`,
        );
            ...
            if (hoveredArea === 1) {
              ...
              dispatch(
                boardChangeCurrentCountByItemId(
                  contextItem.id,
                  contextItem.currentCount - draggedItemRef.current.currentCount,
                ),
              );
          ...
          // ------------------------------------- Add to board -------------------------------------
            ...
            // ------------------------------------------- Add from board (to board) ------------------------------
              ...
              allHoveredSquaresRef.current.forEach((hovSquare) => {
                // @ts-ignore
                if (
                  boardCellsRef.current[hovSquare[1]][hovSquare[0]] &&
                  boardCellsRef.current[hovSquare[1]][hovSquare[0]].id === contextItem.id
                ) {
          ...
          // ------------------------------------- Add to external board -------------------------------------
            ...
            // Add from external board to external board
              ...
              allHoveredSquaresRef.current.forEach((hovSquare) => {
                // @ts-ignore
                if (
                  externalBoardCellsRef.current[hovSquare[1]][hovSquare[0]] &&
                  externalBoardCellsRef.current[hovSquare[1]][hovSquare[0]].id === contextItem.id
                ) {
                  isPartOfItemHovered = true;
                }
              });
    ...
    return (
      <div
        className={classes.RangeComponentWrapper}
        style={styles}
        ref={containerRef}
        ...
        <div className={classes.RangeComponentBG}>
          <div className={classes.InputRangeWrapper}>
            <input
              ...
              value={splittedCount}
              onChange={splittedCountChangeHandler}
              min={1}
              max={contextItem && contextItem.currentCount - 1}
              step={1}
            />
          </div>
          <div className={classes.SplitCurrentCount}>{splittedCount}</div>
          <div className={classes.ButtonWrapper}>
            <div className={classes.CancelButton} onClick={cancelButtonMouseDownHandler}>
              Отмена
            </div>
            <div className={classes.SuccessButton} onClick={successButtonMouseDownHandler}>
              Ок
            </div>
          </div>
        </div>
