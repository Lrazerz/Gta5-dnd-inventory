...
const openContextMenu = (item, rect, hoveredArea) => {
  return (dispatch) => {
    ...
    if (hoveredArea === 3) {
      averX =
        item.mainCell === 1 || item.mainCell === 2 || item.mainCell === 3
...
const _removeBoardItemHandler = (item) => {
  ...
  if (
    category === ItemTypes.WEAPON_RIFLE ||
    category === ItemTypes.WEAPON_PISTOL ||
...
export { openContextMenu, closeContextMenu, getContextActionsForCell };
