...
const stackItem = (fromSplit = false) => {
  return (dispatch, getState) => {
    ...
    if (fromSplit) {
      ...
      if (draggedItemArea === 1) {
        const {currentCount: srcItemcCount} = getState().inventory.board.board[draggedItem.mainCell[1]][draggedItem.mainCell[0]];
        ...
        const {currentCount: srcItemcCount} =
          getState().inventory.externalBoard.externalBoard[draggedItem.mainCell[1]][draggedItem.mainCell[0]];
        const stackedCount = draggedItem.currentCount - draggedItemNewCurrentCount;
        draggedItemNewCurrentCount = srcItemcCount - stackedCount;
        dispatch(externalBoardChangeCurrentCountByItemId(draggedItem.id, draggedItemNewCurrentCount));
      }
        ...
        const {currentCount: srcItemcCount} =
          getState().inventory.equippedItems.cells[draggedItem.mainCell].item;
        const stackedCount = draggedItem.currentCount - draggedItemNewCurrentCount;
        draggedItemNewCurrentCount = srcItemcCount - stackedCount;
        dispatch(equippedChangeCurrentCount(draggedItem.mainCell, draggedItemNewCurrentCount));
      }
...
const dragEndHandler = () => {
  return (dispatch, getState) => {
    const {canDrop, goingToStack, goingToDrop, item: draggedItem, hoveredSquare, hoveredArea, draggedItemArea} = getState().inventory.draggedItem;
