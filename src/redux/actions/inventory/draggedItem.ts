...
const _setHoveredSquares = (squareCoords, allHoveredSquares, canDrop, hoveredArea, goingToStack = null) => {
  return {
    type: HOVERED_SQUARES_SET,
    square: squareCoords,
    squares: allHoveredSquares,
    canDrop,
...
let _getItemOffsets: (
  itemWidth: number,
  itemHeight: number,
) => { xUp: number; xDown: number; yUp: number; yDown: number };
...
let addDraggedItem: (
  item: ItemModel,
  draggedItemArea: number,
  fromRotate?: boolean,
  prevGoingToDropAreaId?: number | null,
  prevHoveredArea?: number | null,
  prevHoveredSquare?: [number, number] | number | null,
) => void;
addDraggedItem = (
  item,
  draggedItemArea,
  fromRotate = false,
  goingToDropAreaId = null,
  prevHoveredArea = null,
  prevHoveredSquare = null,
) => {
  return (dispatch) => {
...
const _getHoveredSquares = (
  hoveredSquare: [number, number],
  xUp: number,
  xDown: number,
  yUp: number,
  yDown: number,
...
const setHoveredSquares = (hoveredSquare, hoveredArea = 1) => {
  return (dispatch, getState) => {
    ...
    let stackableItemNewCurrentCount = 0;
    // indicates should remove draggedItem
    let draggedItemNewCurrentCount = 0;
    ...
    if (hoveredArea === 1) {
      ...
      allHoveredSquares.forEach((hoveredSquare) => {
        ...
        if (hoveredX < xMin || hoveredX > xMax || hoveredY < yMin || hoveredY > yMax) {
          ...
          // id check just to set up canDrop = false
          ...
          const isAllowToStack =
            boardSquare.name === item.name &&
            boardSquare.id !== item.id &&
            boardSquare.currentCount &&
            boardSquare.maxCount &&
    ...
    //region ------------------------------ If equipped hovered ----------------------------------------
      ...
      if (allowedCategory !== item.category) {
        ...
        const isAllowToStack =
          equippedSquareItem.name === item.name &&
          equippedSquareItem.id !== item.id &&
          equippedSquareItem.currentCount &&
          equippedSquareItem.maxCount &&
  ...
  return (dispatch, getState) => {
    ...
    const {
      draggedItem: {
        ...
        goingToStack: {
          stackableItem,
          stackableItemNewCurrentCount: srcStackableItemNewCurrC,
          draggedItemNewCurrentCount: srcDraggedItemNewCurrC,
        },
      },
    } = getState().inventory;

    const stackableItemNewCurrentCount: number = srcStackableItemNewCurrC;
    ...
    if (fromSplit) {
      ...
      if (draggedItemArea === 1) {
        const { currentCount: srcItemcCount } = getState().inventory.board.board[draggedItem.mainCell[1]][
          draggedItem.mainCell[0]
        ];
        ...
        const { currentCount: srcItemcCount } = getState().inventory.externalBoard.externalBoard[
          draggedItem.mainCell[1]
        ][draggedItem.mainCell[0]];
        const stackedCount = draggedItem.currentCount - draggedItemNewCurrentCount;
        draggedItemNewCurrentCount = srcItemcCount - stackedCount;
        dispatch(externalBoardChangeCurrentCountByItemId(draggedItem.id, draggedItemNewCurrentCount));
      } else if (draggedItemArea === 3) {
        const { currentCount: srcItemcCount } = getState().inventory.equippedItems.cells[draggedItem.mainCell].item;
        const stackedCount = draggedItem.currentCount - draggedItemNewCurrentCount;
        draggedItemNewCurrentCount = srcItemcCount - stackedCount;
        dispatch(equippedChangeCurrentCount(draggedItem.mainCell, draggedItemNewCurrentCount));
      }
...
const rotateItem = () => {
  return (dispatch, getState) => {
    const oldDraggedItemInfo = { ...getState().inventory.draggedItem };
    ...
    if (oldDraggedItemInfo.goingToDrop) {
      dispatch(addDraggedItem({ ...oldDraggedItem }, 0, oldDraggedItemInfo.goingToDrop.areaId));
    }
    //endregion
    //save other data
    else {
      dispatch(
        addDraggedItem(
          { ...oldDraggedItem },
          oldDraggedItemInfo.draggedItemArea,
          true,
          null,
          oldDraggedItemInfo.hoveredArea,
...
const rotateItemOnBoard = (item, hoveredItemArea) => {
  ...
  return (dispatch, getState) => {
    ...
    //region ------------------------------ Rotate external board item ------------------------------
      ...
      const { externalBoard } = getState().inventory.externalBoard;
      let canDrop = true;
...
let _moveToEquippedItems: (
  dispatch: (any) => void,
  getState: () => any,
  draggedItem: ItemModel,
  draggedItemArea: number,
...
_moveToEquippedItems = (dispatch, getState, draggedItem, draggedItemArea, hoveredSquare) => {
  if (draggedItemArea === 1) {
    if (
      draggedItem.category === ItemTypes.WEAPON_RIFLE ||
      draggedItem.category === ItemTypes.WEAPON_PISTOL ||
    ...
    if (
      ...
      const _getSuitableSpace = (itemHeight, itemWidth) => {
        const board = getState().inventory.board.board;
        let breakExternalBoardLoop = false;
        let breakExternalLoop = false;
      ...
      if (newMainCellOnBoard) {
        ...
        dispatch(setEquippedItem(hoveredSquare));
        dispatch(
          addItemBySquares(squares, {
            ...draggedItem,
            mainCell: newMainCellOnBoard,
...
const dragEndHandler = () => {
  return (dispatch, getState) => {
    const {
      ...
      hoveredArea,
      draggedItemArea,
    } = getState().inventory.draggedItem;
