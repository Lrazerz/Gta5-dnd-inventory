...
const Alerts: React.FC = React.memo(() => {
  ...
  useEffect(() => {
    ...
    if (alerts) {
      if (alertsWithDisappearing.length === 0 || alerts.length === 3) {
        ...
        const disappearedElements: AlertWithDisappearingInterface[] = [];
        for (let i = 0; i < alertsWithDisappearing.length; i++) {
          ...
          if (newAlertsIdx === -1) {
            ...
            // element disappeared
            disappearedElements.push({
              ...alertsWithDisappearing[i],
              isDisappearing: true,
            });
          }
        }
  ...
  const alertsBlock: JSX.Element[] =
    alertsWithDisappearing.length > 0 &&
    alertsWithDisappearing.map((alertWithDisappearing) => {
      return (
  ...
  return <div className={classes.Alerts}>{alertsBlock}</div>;
});
