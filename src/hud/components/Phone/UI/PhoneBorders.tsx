...
const PhoneBorders: React.FC<Props> = React.memo(function PhoneBorders({ themeFromServer, children }) {
      ...
      if (themeFromServer) {
        ...
        try {
          importedThemeImage = await import(
            `../../../../assets/hud/images/components/Phone/themes/${cosmeticsSettings.themeImage}.svg`
          );
  ...
  const displayBlur = openedScreen === OpenedScreenEnum.incomingCall || openedScreen === OpenedScreenEnum.currentCall;
