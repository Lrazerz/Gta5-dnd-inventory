...
import { ChatMessageInterface, DateObjectInterface, ThemesEnum } from '../../../../models/hud/phone/reducerInterfaces';
...
const ChatMessage: React.FC<Props> = React.memo(({ direction, date, message, playerImage, theme }) => {
  ...
  const _messageTextStyles =
    direction === 'in'
      ? theme === ThemesEnum.black
        ? phoneTheme.white
        : phoneTheme.darkPurple
      : // ?
        phoneTheme.white;
  ...
  const _messageBgStyles =
    direction === 'in'
      ? theme === ThemesEnum.black
        ? phoneTheme.superLightBlack
        : '#fff'
      : theme === ThemesEnum.black
      ? phoneTheme.darkPurple
      : phoneTheme.chatMessageLightPurpleBg;
