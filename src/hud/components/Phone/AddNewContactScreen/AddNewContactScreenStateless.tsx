...
const AddNewContactScreenStateless: React.FC<Props> = React.memo(function AddNewContactScreenStateless({
  allImportedImages,
  isDropDownOpened,
  selectedImage,
  nameText,
  ...
  const previewImageStyles: CSSProperties = {
    border: selectedImage
      ? `0.1rem solid 
    ${theme === ThemesEnum.black ? phoneTheme.lightPurple : 'rgba(148, 150, 157, 0.5)'}`
      : 'none',
  };
  ...
  const avatarChooseButtonBlock = (
    <div className={classes.AvatarChooseButtonWrapper} onClick={onDropDownOpen}>
      <LeadText styles={chooseAvatarTextStyles}>Выбрать аватар</LeadText>
    </div>
  );
  ...
  const dropDownBlock = (
    <div className={classes.DropDownContainer} style={dropDownContainerStyles}>
      <div className={classes.ImagesGridWrapper}>{imagesBlock}</div>
    </div>
  );

  const avatarChooseBlock = isDropDownOpened ? dropDownBlock : avatarChooseButtonBlock;
  ...
  return (
    <div className={classes.AddNewContactScreen} {...handlers} onClick={onDropDownClose}>
      ...
      <div className={classes.AvatarChooseButtonContainer}>{avatarChooseBlock}</div>
      <div className={classes.HorizontalLine} style={horizontalLineStyles} />
      ...
      <div className={classes.AddContactButtonContainer}>
        <div className={classes.AddContactButton} onClick={onAddContact}>
          <LeadText styles={addContactTextStyles}>Добавить контакт</LeadText>
        </div>
      </div>
    </div>
  );
