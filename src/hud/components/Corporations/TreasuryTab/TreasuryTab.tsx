...
import { mpTrigger_corporations_treasury_transfer } from '../../../../utils/mpTriggers/hud/corporations/tabs/treasury/treasuryTriggers';
...
const TreasuryTab: React.FC<Props> = React.memo(() => {
  ...
  const treasuryTabRedux: TreasuryInitialStateInterfaceWithLoading = useSelector(
    (state) => state.hud.corporations.tabs.treasury,
  );

  const [availableAmountToday, setAvailableAmountToday]: [
    AvailableAmountTodayType,
    (AvailableAmountTodayType) => void,
  ...
  return (
    <TreasuryTabStateless
      treasuryTabData={treasuryTabRedux}
      isWithdrawMenuOpened={isWithdrawMenuOpened}
      onWithdrawMenuOpen={openWithdrawMenuHandler}
      onWithdrawMenuClose={closeWithdrawMenuHandler}
      isPutMenuOpened={isPutMenuOpened}
      onPutMenuOpen={openPutMenuHandler}
      onPutMenuClose={closePutMenuHandler}
      onTransfer={transferHandler}
      availableAmountToday={availableAmountToday}
      ...
      onAvailableAmountBlur={changeAvailableAmountRedux}
      nicknameToTransfer={nicknameToTransfer}
      onNicknameToTransferChange={changeNicknameToTransferHandler}
      amountToTransfer={amountToTransfer}
      onAmountToTransferChange={changeAmountToTransferHandler}
