...
import { treasuryMinAmountToWithdraw } from '../../../../constants/hud/corporations/treasury/treasury';
...
const TreasuryWithdrawMenu: React.FC<Props> = React.memo(() => {
  ...
  const [withdrawAmount, setWithdrawAmount]: [WithdrawAmountType, (WithdrawAmountType) => void] = useState(
    treasuryMinAmountToWithdraw,
  );

  const withdrawAvailableTodayRedux = useSelector(
    (state) => state.hud.corporations.tabs.treasury.withdrawAvailableToday,
  );
  ...
  return (
    <TreasuryWithdrawPutPopUp
      title={'Введите сумму'}
      tipTitle={'Возможно взять из казны сегодня'}
      tipNumber={withdrawAvailableTodayRedux}
      inputNumber={splitStringDigits(withdrawAmount)}
      onInputChange={changeWithdrawAmountHandler}
      onButtonClick={withdrawMoneyHandler}
