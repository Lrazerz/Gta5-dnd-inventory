...
import {
  ...
  LogsSearchByEnumEng,
  LogsSearchByRussian,
} from '../../../../models/hud/corporations/tabs/logs/logsEnums';
...
const LogsTitleSearch: React.FC<Props> = React.memo((props) => {
  ...
  const [searchByFilter, setSearchByFilter]: [string, (string) => void] = useState(
    LogsSearchByDict[LogsSearchByEnumEng[0]],
  );

  // изменяется ток когда изменяется стейт
  const [potentialsSearchBy, setPotentialsSearchBy]: [string[], (any) => void] = useState(
    LogsSearchByRussian.filter((item) => item !== searchByFilter),
  );
  ...
  return (
    <div className={classes.LogsTitleSearch}>
      <CorporationsText styles={titleTextStyles}>Журнал действий</CorporationsText>
      ...
      <div className={classes.SearchByWrapper}>
        <div className={classes.SearchByTitleAndDropdown}>
          <div className={classes.SearchByTitle}>
            <CorporationsText styles={searchByTitleStyles}>Поиск по</CorporationsText>
          </div>
          <div className={classes.SearchByDropdownContainer}>
            <CorporationsDropdown
              selectedItem={searchByFilter}
              list={potentialsSearchBy}
              onSelect={searchByPickHandler}
