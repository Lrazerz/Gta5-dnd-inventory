...
} from "../../../../models/corporations/tabs/tasks/tabs/newTaskInterfaces";
...
import {
  ...
  minTaskSumMessage,
  minTaskTitleLength,
  minTaskTitleLengthMessage, noTaskExecutorMessage, noTypedTaskExecutorMessage,
...
const pickExecutor = (executor: TasksPotentialExecutorInterface) => {
  return {type: PICK_EXECUTOR, executor: executor.executor, executorType: executor.executorType};
}
...
const NewTaskTab: React.FC<Props> = React.memo(() => {
  ...
  const potentialExecutors: TasksPotentialExecutorInterface[] =
    useSelector(state => state.hud.corporations.tabs.tasks.tabs.new.potentialExecutors);
  ...
  const [filteredPotentialExecutors, setFilteredPotentialExecutors]: [TasksPotentialExecutorInterface[], any] = useState();
  ...
  const createNewTaskHandler = () => {
    ...
    if (reactState.inputValues[StateFieldsEnum.title].length < minTaskTitleLength) {
      ...
      isValid = false;
      reactDispatch(changeIsValidAction(StateFieldsEnum.title, false,
    ...
    if (reactState.inputValues[StateFieldsEnum.sum] < minTaskSum) {
      ...
      isValid = false;
      reactDispatch(changeIsValidAction(StateFieldsEnum.sum, false,
        minTaskSumMessage));
    ...
    if (reactState.inputValues[StateFieldsEnum.description].length < minTaskDescriptionLength) {
      ...
      isValid = false;
      reactDispatch(changeIsValidAction(StateFieldsEnum.description, false,
        minTaskDescriptionLengthMessage));
  ...
  return (
    <NewTaskTabStateless onContainerClick={containerClickHandler} isLoading={isLoading}
                         taskTitle={reactState.inputValues[StateFieldsEnum.title]}
                         onTaskTitleChange={changeTaskTitleHandler} taskSum={reactState.inputValues[StateFieldsEnum.sum]}
                         onTaskSumChange={changeTaskSumHandler} onTaskSumBlur={sumBlurHandler}
                         taskExecutor={reactState.inputValues[StateFieldsEnum.executor]}
                         filteredPotentialExecutors={filteredPotentialExecutors}
                         onTaskExecutorChange={changeExecutorHandler} onTaskExecutorFocus={executorFocusHandler}
                         ...
                         onTaskDescriptionChange={changeTaskDescriptionHandler} onTaskCreate={createNewTaskHandler}/>
  );
});
