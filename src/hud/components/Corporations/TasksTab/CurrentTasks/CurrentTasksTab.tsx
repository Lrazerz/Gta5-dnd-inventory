  ...
  useEffect(() => {
    ...
    if (!window.corporations_tasks_currentTasks_openPage) {
      ...
      window.corporations_tasks_currentTasks_openPage = (jsonData: string) => {
        const parsedData: TasksCurrentTasksInitialStateInterface = window_corporations_tasks_currentTasks_openPage(
          jsonData,
        );
      ...
      <div className={classes.TasksTab}>
        ...
        <div className={classes.ActionButtonsContainer}>
          <div className={classes.ActionButton}>
            <CorporationsButton
              onClick={() => changeTasksTabHandler(CorporationsTasksTabsEnumEng.newTask)}
              title={'Новая задача'}
              imageUrl={newTaskImg}
              styles={buttonStyles}
              imageWidth={'17%'}
              titleTextStyles={newTaskButtonTitleTextStyles}
            />
          </div>
          <div className={classes.ActionButton}>
            <CorporationsButton
              onClick={() => changeTasksTabHandler(CorporationsTasksTabsEnumEng.archiveTasks)}
              title={'Архив задач'}
              imageUrl={tasksArchiveImg}
              styles={buttonStyles}
              imageWidth={'17%'}
              titleTextStyles={archiveButtonTitleTextStyles}
            />
          </div>
        </div>
