...
const PropertyOfficeTab: React.FC<Props> = React.memo(() => {
  ...
  const optionsRedux = useSelector(
    (state) => state.hud.corporations.tabs.permissions.tabs.property.tabs.office.options,
  );
  const selectedRole: SingleRoleInterface = useSelector(
  ...
  return <div className={classes.PropertyOfficeTab}>{optionsBlock}</div>;
});
