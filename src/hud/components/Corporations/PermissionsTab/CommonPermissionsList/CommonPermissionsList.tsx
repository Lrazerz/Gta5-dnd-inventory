...
const CommonPermissionsList: React.FC<Props> = React.memo(() => {
  ...
  const [filteredPermissionsSets, setFilteredPermissionsSets]: [CommonPermissionsSetInterface[], any] = useState();

  const permissionsSetsRedux = useSelector(
    (state) => state.hud.corporations.tabs.permissions.permissions.commonPermissionsSets,
  );
          ...
          for (let j = 0; j < permissionsSets[i].permissions.length; j++) {
            if (permissionsSets[i].permissions[j].title.toLowerCase().includes(searchText.toLowerCase())) {
              ...
              if (addedSetIdx !== -1) {
                ...
                resultFilteredSets.push({
                  ...permissionsSets[i],
                  permissions: [permissionsSets[i].permissions[j]],
                });
              }
            }
          }
    ...
    <div className={classes.CommonPermissionsList}>
      ...
      <div className={classes.InputWrapper}>
        <CorporationsInput
          ...
          maxLength={maxCommonPermissionLength}
          value={searchText}
      ...
      <div className={classes.PermissionsSetsWrapper}>
        {permissionsSets &&
          filteredPermissionsSets &&
          filteredPermissionsSets.map((permSet) => (
            <CommonPermissionsSet
