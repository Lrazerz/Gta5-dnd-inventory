...
const InteractionsContainerStateless: React.FC<Props> = React.memo(({crossRef, onClose, onPickInteraction,
                                                           dimensions, interactionsArrays}) => {
  ...
  const headerInteractionsBlock = dimensions && interactionsArrays.headerInteractions.length > 0 && (
    <div style={headerBlockStyles} className={classes.Header}>
      <div  style={baseSingleInteractionStyles} className={classes.BaseSingleInteractionWrapper}>
        <SingleInteraction name={interactionsArrays.headerInteractions[0].name}
                           isReversed={false} isCentral={true} onPickInteraction={onPickInteraction}
  ...
  const baseInteractionsBlock = dimensions && interactionsArrays.baseInteractions.length > 0 && (
    <div className={classes.Base}>
      {interactionsArrays.baseInteractions.map((interaction, i) => {
        const isElLastInTheRow: boolean = dimensions.columnsCount === 2 ?
          (i+1) % 2 === 0 : false;
        const isCentral: boolean = dimensions.columnsCount === 3 ?
          (i+1) % 3 === 2 : false;
        ...
        return (
          <div key={interaction.name} style={baseSingleInteractionStyles} className={classes.BaseSingleInteractionWrapper}>
            <SingleInteraction name={interaction.name} isCentral={isCentral} isAlignedRight={isAlignedRight}
                               onPickInteraction={onPickInteraction}
                               isReversed={isElLastInTheRow} Enabled={interaction.enabled}/>
          </div>
        )
      })}
    </div>
  ...
  const footerInteractionsBlock = dimensions && interactionsArrays.footerInteractions.length > 0 && (
    <div style={footerBlockStyles} className={classes.Footer}>
      <div  style={baseSingleInteractionStyles} className={classes.BaseSingleInteractionWrapper}>
        <SingleInteraction name={interactionsArrays.footerInteractions[0].name}
                           isReversed={false} isCentral={true} onPickInteraction={onPickInteraction}
