...
const SingleInteraction: React.FC<Props> = React.memo(
  ({ name, isReversed, isCentral, Enabled, isAlignedRight = false, onPickInteraction }) => {
    ...
    useEffect(() => {
      ...
      const loadThemeImage = async () => {
        if (name) {
          ...
          try {
            importedThemeImage = await import(
              `../../../assets/hud/images/components/Interactions/${name.toLowerCase()}.svg`
            );
            if (!isCanceled) {
            ...
            if (e.message.startsWith('Cannot find')) {
              ...
              const defaultInteractionName = 'default-interaction';
              importedThemeImage = await import(
                `../../../assets/hud/images/components/Interactions/${defaultInteractionName}.svg`
              );
              if (!isCanceled) {
                setImportedInteractionImage(importedThemeImage.default);
              }
    ...
    useEffect(() => {
      if (fullContainerRef.current) {
        ...
        setDimensions({
          textBlockPadding: textBlockPadding + 'px',
    ...
    return (
      <SingleInteractionStateless
        ...
        interactionImage={importedInteractionImage}
        onClick={pickInteractionHandler}
        onMouseOver={hoverInterationHandler}
        onMouseLeave={mouseLeaveInteractionHandler}
